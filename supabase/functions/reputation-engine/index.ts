...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ---------- Compute Effective Reporter Trust ----------
    ...
    // 2. Count total actions (reports + votes)
    ...
    // Fetch action timestamps and apply exponential decay
    ...
    for (const action of recentActions ?? []) {
      const ageDays =
        (now - new Date(action.created_at).getTime()) / (1000 * 60 * 60 * 24);
      const decayFactor = Math.exp(-decayConstant * ageDays);
      decayedReputationSum += (action.weight ?? 1) * decayFactor;
    }

    // 7. Diminishing Returns: after 50 actions, weight reduced by 1/ln(n)
    let diminishingFactor = 1.0;
    if (n > 50) {
      diminishingFactor = 1 / Math.log(n);
    }

    // 8. Final Effective Trust
    // E_trust = C(n) * rawReputation * maturity * diminishing
    const effectiveTrust = Math.min(
      100,
      Math.max(
        0,
        confidence * rawReputation * maturityMultiplier * diminishingFactor
      )
    );

    // 9. Update profile with computed scores
    await supabase
      .from("profiles")
      .update({
    ...
    return new Response(
      JSON.stringify({
        ...
        total_actions: n,
        decayed_reputation_sum:
          Math.round(decayedReputationSum * 100) / 100,
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (err) {
